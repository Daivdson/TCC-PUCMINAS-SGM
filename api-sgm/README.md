...
1. Rodar direto da IDE a classe `ApiPesquisaApplication`, lembrando de informar o profile para a jvm com `-Dspring.profiles.active=dev`

2. (Alternativa) `mvn spring-boot:run -Drun.profiles=dev`

3. Acessar [http://localhost:8082/docs/](http://localhost:8082/docs/)
...
Não deve ser usando nesse repositório bibliotecas ou implementações para fazer consultas no elasticSearch, para isso deve ser usado o repositório citado acima. Nele já possui métodos e formas padrões para fazer essas consultas, tirando complexidades e códigos duplicados para consulta com ElasticSearch. Exemplo de uso do [NoticiaRest](src/main/java/br/leg/camara/apipesquisa/adaptadores/repositorio/rest/NoticiaRest.java).
